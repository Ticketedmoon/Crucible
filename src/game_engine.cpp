...
void GameEngine::update(double& currentTime, double& accumulator, size_t& totalFrames)
{
    ...
    while (accumulator >= Crucible::DT)
    {
        handleInput();
        gameScenes[currentScene]->update();
        accumulator -= Crucible::DT;
    }
...
{
    window.create(sf::VideoMode(Crucible::WINDOW_WIDTH, Crucible::WINDOW_HEIGHT), Crucible::WINDOW_TITLE.data());

    uint32_t screenWidth = sf::VideoMode::getDesktopMode().width;
    uint32_t screenHeight = sf::VideoMode::getDesktopMode().height;
    window.setPosition(sf::Vector2i((screenWidth - Crucible::WINDOW_WIDTH)/2, (screenHeight - Crucible::WINDOW_HEIGHT)/2));

    if (Crucible::USE_VERTICAL_SYNC)
    {
    ...
    renderTexture.create(Crucible::WINDOW_WIDTH, Crucible::WINDOW_HEIGHT);
    renderSprite.setTexture(renderTexture.getTexture());
    renderSprite.setTextureRect(sf::IntRect(0, 0, Crucible::WINDOW_WIDTH, Crucible::WINDOW_HEIGHT));
}
